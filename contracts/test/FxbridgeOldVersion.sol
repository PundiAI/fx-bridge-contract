...
contract FxbridgeOldVersion is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    enum BridgeStatus {Invalid, Pause, Active, Originated}
    mapping(address => BridgeStatus) public state_bridgeTokens;
    ...
    function addBridgeToken(address _tokenAddr, bytes32 _channelIBC, bool _isOriginated) public onlyOwner returns (bool)  {
        ...
        require(state_bridgeTokens[_tokenAddr] == BridgeStatus.Invalid, "Bridge token already exists");
        if (_isOriginated) {
            state_bridgeTokens[_tokenAddr] = BridgeStatus.Originated;
        }else{
            state_bridgeTokens[_tokenAddr] = BridgeStatus.Active;
        }
    ...
    function pauseBridgeToken(address _tokenAddr) public onlyOwner returns (bool) {
        require(_tokenAddr != address(0), "Invalid address");
        require(state_bridgeTokens[_tokenAddr] != BridgeStatus.Invalid, "Bridge token already exists");
        state_bridgeTokens[_tokenAddr] = BridgeStatus.Pause;
        return true;
    }
    ...
    function sendToFx(address _tokenContract, bytes32 _destination, bytes32 _targetIBC, uint256 _amount) public nonReentrant whenNotPaused {
        BridgeStatus bridgeStatus = state_bridgeTokens[_tokenContract];
        require(bridgeStatus == BridgeStatus.Active || bridgeStatus == BridgeStatus.Originated, "Unsupported token address");
        IBEP20MetadataUpgradeable(_tokenContract).safeTransferFrom(msg.sender, address(this), _amount);

        if (bridgeStatus == BridgeStatus.Originated) {
    ...
    function checkAssetStatus(address _tokenAddr) public view returns (bool){
        return state_bridgeTokens[_tokenAddr] == BridgeStatus.Active || state_bridgeTokens[_tokenAddr] == BridgeStatus.Originated;
    }
